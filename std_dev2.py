...
STD_DEV_AWAY_THRESHOLD = 2.45 # Configurable threshold for Std Devs Away flag
...
plot_std_dev_flags_x = deque(maxlen=PLOT_HISTORY) # For storing x-coordinates of flags
plot_std_dev_flags_y = deque(maxlen=PLOT_HISTORY) # For storing y-coordinates of flags
                        ...
                        with lock:
                            ...
                            if wlr_value is not None:
                                ...
                                if std_dev != 0:
                                    ...
                                    # --- Check for Std Dev Away Threshold and add flag ---
...
# New plot element for the flags
scatter_std_dev_flags, = ax.plot([], [], 'go', markersize=8, label=f'Std Devs Away >= $\\pm${STD_DEV_AWAY_THRESHOLD}', alpha=0.7) # Green circles
...
def init_plot():
    ...
    line_price.set_data([], [])
    line_wlr.set_data([], [])
    scatter_std_dev_flags.set_data([], []) # Initialize flag data
    return line_price, line_wlr, scatter_std_dev_flags # Return all artists
...
def update_plot(frame):
    ...
    with lock: # Access shared data safely
        ...
        x_flags_data = list(plot_std_dev_flags_x) # Get flag x-data
        y_flags_data = list(plot_std_dev_flags_y) # Get flag y-data
    ...
    scatter_std_dev_flags.set_data(x_flags_data, y_flags_data) # Update flag data
